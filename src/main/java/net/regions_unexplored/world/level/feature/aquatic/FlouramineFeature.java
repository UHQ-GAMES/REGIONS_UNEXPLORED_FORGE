      ...
      if (worldgenlevel.getBlockState(blockpos1).is(Blocks.WATER)) {
         ...
         for(int l = 0; l <= k; ++l) {
            if (worldgenlevel.getBlockState(blockpos1).is(Blocks.WATER) && worldgenlevel.getBlockState(blockpos1.above()).is(Blocks.WATER) && blockstate1.canSurvive(worldgenlevel, blockpos1)) {
               if (l == k) {
                  worldgenlevel.setBlock(blockpos1, blockstate.setValue(FlouramineBlock.AGE, Integer.valueOf(randomsource.nextInt(4) + 20)), 5);
                  worldgenlevel.scheduleTick(blockpos1, blockstate.getBlock(), 5);
                  ++i;
               } else {
